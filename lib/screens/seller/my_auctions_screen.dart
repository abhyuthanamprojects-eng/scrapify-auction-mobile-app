...
class SellerMyAuctionsScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<SellerMyAuctionsScreen> createState() =>
      _SellerMyAuctionsScreenState();
}

class _SellerMyAuctionsScreenState
    extends ConsumerState<SellerMyAuctionsScreen> {
  String _filter = 'all';

  @override
  Widget build(BuildContext context) {
    final isPending = ref.watch(authProvider).user?.isKycPending ?? false;
              ...
              data: (auctions) {
                final filtered = _filter == 'all'
                    ...
                    : auctions
                          .where(
                            (a) =>
                                a.status == AuctionStatus.fromString(_filter),
                          )
                          .toList();
                if (filtered.isEmpty) {
                  return EmptyState(
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      auction.materialType ?? auction.category ?? auction.title,
          ...
          if (auction.status == AuctionStatus.sentBack &&
              auction.reviewComment != null) ...[
            const SizedBox(height: 8),
            Container(
