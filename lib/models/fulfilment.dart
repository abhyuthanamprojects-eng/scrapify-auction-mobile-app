...
class FulfilmentStage extends Equatable {
  ...
  factory FulfilmentStage.fromJson(Map<String, dynamic> json) =>
      FulfilmentStage(
...
class GatePassData extends Equatable {
  ...
  factory GatePassData.fromJson(Map<String, dynamic> json) => GatePassData(
    passId:
        json['pass_id'] as String? ??
        'GP-${DateTime.now().millisecondsSinceEpoch}',
...
class FulfilmentRecord extends Equatable {
  ...
  String get status =>
      deliveryStatus ??
      (currentStageIndex >= stages.length ? 'COMPLETED' : 'IN_FULFILMENT');
  String get auctionTitle => title;
  String get sellerCompany => '';
  double get totalAmountInr => 0;
  String get currentStage =>
      (currentStageIndex < stages.length && stages.isNotEmpty)
